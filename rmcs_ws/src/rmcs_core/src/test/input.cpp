...
public:
    ...
    void update() override {
        ...
        if ((*remote_left_switch_ == Switch::DOWN || *remote_left_switch_ == Switch::UNKNOWN)
            ...
            *motor_control_velocity_right= 15 * remote_left_joystic_->x();
            *motor_control_velocity_left= 15 * remote_left_joystic_->x();

            RCLCPP_INFO(get_logger(), "motor_control_velocity_right:%lf", *motor_control_velocity_right);
            RCLCPP_INFO(get_logger(), "motor_control_velocity_left:%lf", *motor_control_velocity_left);
